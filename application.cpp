...
void Application::parseCommandLine(const int argc, const char* const argv[]) const noexcept {
    ...
    po::parsed_options parsed =
        po::command_line_parser(argc, argv).options(mainDesc).allow_unregistered().run();
...
void Application::initTrainingMode(const po::variables_map& vm) const noexcept {
    ...
    if (vm.count("info")) {
        std::cout << "info = " << vm["info"].as<std::string>() << std::endl;
    }
...
void Application::initRecognitionMode(const po::variables_map& vm) const noexcept {
    ...
    if (vm.count("params")) {
        std::cout << "params = " << vm["params"].as<std::string>() << std::endl;
    }
