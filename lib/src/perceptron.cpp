...
std::vector<std::vector<double>> NeuroNetwork::forward(const std::vector<double>& aInput) {
...
void NeuroNetwork::train(const std::vector<std::vector<double>>& aInputData,
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t sample = 0; sample < aInputData.size(); ++sample) {
            ...
            std::vector<std::vector<double>> activations = forward(aInputData[sample]);
            ...
            for (int i = static_cast<int>(m_layers.size()) - 1; i >= 0; --i) {
                ...
                for (size_t j = 0; j < m_layers[i].size(); ++j) {
                    if (i == m_layers.size() - 1) {
                        // output layer
                        double error = aTargetData[sample][j] - activations[i + 1][j];
                        deltas[i][j] = error * m_layers[i][j].activateDerivative(activations[i + 1][j]);
            ...
            for (size_t i = 0; i < m_layers.size(); ++i) {
                for (size_t j = 0; j < m_layers[i].size(); ++j) {
                    m_layers[i][j].updateWeights(activations[i], aLearningRate, deltas[i][j]);
                }
            }
        }
